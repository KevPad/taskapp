...
exports.searchTasks = (req, res) => {
    ...
    if (tipoUsuario === 'ADMIN') {
        // Si el usuario es ADMIN, busca en todas las tareas
        query = 'SELECT * FROM task WHERE nombre_task LIKE ?';
        queryParams = ['%' + searchQuery + '%'];
    } else {
        // Si el usuario es GENERAL, solo busca sus propias tareas
        query = 'SELECT * FROM task WHERE nombre_task LIKE ? AND user_id = ?';
        queryParams = ['%' + searchQuery + '%', userId];
    }
